...
public class JSoupBaiduSearcher extends AbstractBaiduSearcher {
	...
	private static Searcher searcher = new JSoupBaiduSearcher();
	...
	public SearchResult search(String keyword, int page) {
		...
		try {
			...
			for (int i = 0; i < len; i++) {
				String titleCssQuery = "html body div div div div#content_left div#" + (i + 1 + (page - 1) * pageSize)
						+ ".result.c-container h3.t a";
				String summaryCssQuery = "html body div div div div#content_left div#" + (i + 1 + (page - 1) * pageSize)
	...
	public static void main(String[] args) {
		getBestResult("司马懿有几个儿子","8","九","10");
//		getBestResult("四大美人不包括","钟无艳","孟光","黄月英");
		
		
//		SearchResult searchResult = searcher.search("司马懿有几个儿子", 1);
//		List<Webpage> webpages = searchResult.getWebpages();
//		if (webpages != null) {
//			int i = 1;
//			LOG.info("搜索结果 当前第 " + searchResult.getPage() + " 页，页面大小为：" + searchResult.getPageSize() + " 共有结果数："
...
//		}
	...
	public static void getBestResult(String question, String a, String b, String c) {
		int asum = 0;
		int bsum = 0;
		int csum = 0;
		SearchResult searchResult = searcher.search(question, 1);
		List<Webpage> webpages = searchResult.getWebpages();
		if (webpages != null) {
			int i = 1;
			LOG.info("搜索结果 当前第 " + searchResult.getPage() + " 页，页面大小为：" + searchResult.getPageSize() + " 共有结果数："
